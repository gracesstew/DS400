...
Quiz answers: b, left-skewed, Beta(5,5)
...
Quiz answer: B, posterior data will be between prior and new data
...
1.  Describe and compare both the prior model and likelihood function in words.
    ...
    1.  The prior model has a high density around π=1. Because it is concentrated towards the right side of the graph, it is left-skewed. The likelihood model has a low density around π=0.25. Because it is concentrated towards the left side of the graph, it is right-skewed. They have a drastic disagreement.
...
2.  Describe the posterior model in words. Does it more closely agree with the data (as reflected by the likelihood function) or the prior?
    ...
    1.  The posterior is centered near π=0.75. It moves away from the data evidence (prior) but does not fully agree with the likelihood.

3.  Provide the specific `plot_beta_binomial()` code you would use to produce a similar plot.

```{r}
plot_beta_binomial(60,2,y=5,n=20)
```
