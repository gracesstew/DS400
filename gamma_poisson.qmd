...
Probability of having disease if you had a second positive test -\> Increases to 90%
...
# Provides mean and standard deviation of prior vs posterior 
```
...
-   Now the [Gamma-Poisson Model](https://www.bayesrulesbook.com/chapter-5#:~:text=phone%20call%20situation.-,The%20Gamma%2DPoisson%20Bayesian%20model,-Let)
    ...
    -   Gamma Distribution
        ...
        -   The Gamma distribution has two parameters: **shape (s)** and **rate (r)**.
            ...
            -   (Note: It's like the alpha and beta for the \`plot_binomial\`)
...
# Mode: 0 — E.g., Most likely to get 0 fraud phone calls
```
...
-   New Data
    ...
    -   There we have a Gamma(10,2) prior for λ, the daily rate of calls. Further, on four separate days in the second week of August, we received →y=(y1,y2,y3,y4)=(6,2,2,1) such calls. Thus, we have a sample of n=4 data points with a *total* of 11 fraud risk calls and an *average* of 2.75 phone calls per day:
        -   4 samples: 6 (day 1), 2 (day 2), 2 (day 3), 1 (day 4)
...
```{r}
plot_gamma(shape = 7, rate = 1, mean = TRUE, mode = TRUE)
```

1.  The most common value of λ is 10 and the mean is 12.

```{r}
plot_gamma(shape = 12, rate = 1, mean = TRUE, mode = TRUE)
```
...
```{r}
plot_gamma(shape = 11.5, rate = 2, mode = TRUE)
```
...
```{r}
# Using `plot_gamma` to plot our prior
...
1.  Why is the Poisson model a reasonable choice for our data Yi?
    ...
    -   We are looking at count data (observed number of goals) rather than percentages. So it fits the data type for the Gamma Poisson model.

    -   This generally fits out prior understanding of how many goals can be scored in a soccer game, usually very close to 0.
