...
```{r}
plot_beta(5,11) # 10-60%, low proportion of movies pass this test
```

The Clueless: Use `plot_beta()` to visualize the prior

```{r}
plot_beta(1,1) # unsure
```

The Optimist: Use `plot_beta()` to visualize the prior

```{r}
plot_beta(14,1) # 70-100% of movies pass the bechdel test
```
...
```{r}
plot_beta_binomial(5,11,y=9,n=20) # prior=feminist thought, likelihood=based on data, posterior=mix
```
...
1.  Beta(1.8,1.8) --> centering π on 0.5

2.  Beta(3,2) --> somewhat favoring π>0.5

3.  Beta(1,10) --> strongly favoring π<0.5

4.  Beta(1,3) --> somewhat favoring π<0.5

5.  Beta(17,2) --> strongly favoring π>0.5
...
5.  `alpha = 3, beta = 8, y = 2, n = 4` <----- answer
