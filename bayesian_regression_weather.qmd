...
```{r}
ggplot(weather_WU, aes(x = location, y = temp3pm)) +
  geom_boxplot() +
  labs(title = "temperature (3pm) per location")
```

```{r}
ggplot(weather_WU, aes(x = temp9am)) +
...
2.  If your goal were to maximize the predictive quality of your model and you could only choose *one* predictor to use in your model, would you choose `temp9am` or `location`
    ...
    1.  `temp9am` because we are modeling temperature, displayed by Weather Model 1. It was also the most informative.

3.  Which of the four models produces the best overall predictions?

    1.  Overall Weather Model 3, had the best overall predictions. This is because it had the lowest mean absolute error (MAE).\
