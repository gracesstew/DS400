...
ggplot(football, aes(x = group, y = volume, fill = years)) +
  geom_col() +
  facet_wrap(~years)
...
```{r}
plot_normal_likelihood(y = concussion_subjects$volume, sigma = 0.59) # 0.59 = SD
```
...
-   Ask it to critique your analysis
    ...
    -   Are there alternate/better bayesian methods/models to apply to the question your asking? Think of what we have covered (beta-binomial, gamma-poisson, normal-normal)
        ...
        -   ChatGPT suggests using beta-binomial rather than gamma-poisson when looking at what the probability would be of a bus arriving on time.
    ...
    -   What assumptions are associated with the model you chose? With those assumptions in mind, is the model still a good fit for your question/data?
        ...
        -   This assumes that the count of on-time arrivals could be unbounded. Additionally, each bus run is an independent "arrival event." This could be problematic because the number of on-time buses can't exceed Total_Buses per week, but the Gamma-Poisson model doesn't enforce that. This means that while Gamma-Poisson works mechanically, the Beta-Binomial is a better fit given the assumptions.

    -   Are there real datasets you can use to answer your question opposed to AI generated datasets?

        -   Some publicly available datasets could be from city transit agencies or from the US Department of Transportation. Both of these sources would provide real-time bus performance data or service reliability and performance metrics.
